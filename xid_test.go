...
var IDs = []IDParts{
	IDParts{
		XID{0x4d, 0x88, 0xe1, 0x5b, 0x60, 0xf4, 0x86, 0xe4, 0x28, 0x41, 0x2d, 0xc9},
		1300816219,
		[]byte{0x60, 0xf4, 0x86},
		0xe428,
		4271561,
	...
	IDParts{
		XID{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
		0,
		[]byte{0x00, 0x00, 0x00},
		0x0000,
		0,
	...
	IDParts{
		XID{0x00, 0x00, 0x00, 0x00, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0x00, 0x00, 0x01},
		0,
		[]byte{0xaa, 0xbb, 0xcc},
		0xddee,
		1,
...
func TestIDString(t *testing.T) {
	id := XID{0x4d, 0x88, 0xe1, 0x5b, 0x60, 0xf4, 0x86, 0xe4, 0x28, 0x41, 0x2d, 0xc9}
...
func TestFromString(t *testing.T) {
	...
	want := XID{0x4d, 0x88, 0xe1, 0x5b, 0x60, 0xf4, 0x86, 0xe4, 0x28, 0x41, 0x2d, 0xc9}
	if got != want {
...
func TestID_Compare(t *testing.T) {
	pairs := []struct {
		...
		{IDs[1].id, IDs[0].id, -1},
		{XID{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, IDs[2].id, -1},
		{IDs[0].id, IDs[0].id, 0},
	}
	for _, p := range pairs {
...
var IDList = []XID{IDs[0].id, IDs[1].id, IDs[2].id}

// 测试长度
func TestSorter_Len(t *testing.T) {
	if got, want := sorter([]XID{}).Len(), 0; got != want {
...
func TestSort(t *testing.T) {
	...
	ids = append(ids, IDList...)
	XidSort(ids)
	if got, want := ids, []XID{IDList[1], IDList[2], IDList[0]}; !reflect.DeepEqual(got, want) {
		t.Fail()
	}
}

func TestXid_NewString(t *testing.T) {
	for i := 0; i < 100; i++ {
		uniqueID := NewXid().String()
